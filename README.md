...
1. Takes data from paired CRISPR screens that have been pre-processed by the pgmap software, or any counts table of paired gRNA reads
2. The input data will have CRISPR guide counts that correspond to how well cells grow (or don't grow) when different genes or pairs of genes are disabled
3. gimap can take this data and help identify interesting patterns, like:
...
The gimap package is based on the [code and research from the Berger Lab](https://github.com/FredHutch/GI_mapping)

## pgPEN library design

The `gimap` package is based on the Berger lab's original paired CRISPR knockout library design.

There are four target types of paired guide RNAs (pgRNAs) included in the custom Berger lab pgPEN library:

- `double_targeting` - these pgRNAs knockout two different genes at a time. This is sometimes noted as `"gene_gene"`
- `single_targeting` - these pgRNAs have one single guide RNA (sgRNA) that targets a gene, and one designed to not target any gene. These includes `"gene_ctrl"` and `"ctrl_gene"`, which simply denote the order of the gene and control sgRNA in the pgRNA vector.
- `positive_control` - these pgRNAs target pan-essential genes as defined by the Broad Institute's Cancer Dependency Map. These are single targeting pgRNAs with including `"gene_ctrl"` and `"ctrl_gene"` , but are defined as a 'positive_control' when the gene is an essential gene, e.g. required in most cell lines for survival.
- `negative_control` - these cells have two pgRNAs designed to NOT target any genes. This is noted as `"ctrl_ctrl"`.

For each gene pair in the pgPEN library, e.g. `geneA_geneB`, there are 32 different constructs related to it. `32 = 16 single targeting and 16 double targeting`. 
...
The double target genetic interaction scores are calculated at the construct level and are then summarized using a t-test to see if the the distribution of the set of all double targeting constructs for one gene pair is significantly different than the overall distribution of a negative control distribution of genetic interaction scores calculated from mock double targets. The mock double targets are single targeting constructs compared to corresponding negative control constructs. After multiple testing correction, FDR values are reported. Low FDR value for a double construct means high likelihood of either synthetic lethality (for negative genetic interaction scores) or cooperativity (for positive genetic interaction scores).

#### Expected CRISPR scores

Expected CRISPR scores are calculated based on the pgRNAs included.

For a `double_targeting` construct we would expect the CRISPR score to be the sum of the single targeting CRISPR scores of the constructs that use the same gRNA sequences present in the pgRNA.
```
...
gimap takes in a counts matrix of the pgRNA read counts which is related to the proportion of cells in the pool that contained that pgRNA. This data needs some normalization before CRISPR scores and Genetic Interaction scores can be calculated.
...
2. `Calculate log2 fold change` - This is done by subtracting the log2CPM of each pgRNA for the pre-treatment from each post-treatment sample pgRNA log2CPM.  control is what is highlighted. The pretreatment is the day 0 of CRISPR treatment, before CRISPR pgRNAs have taken effect.
```
log2FC = log2CPM for each sample - pretreatment log2CPM
```
...
Calculate medians based on single and double targeting and subtract these medians from `log2FC adjusted`
