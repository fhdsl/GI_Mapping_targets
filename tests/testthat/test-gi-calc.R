test_that("Test Genetic Interaction score calculations", {
     ...
     results <- data.frame(
       ...
       intercept = as.numeric(round(c(-0.263, 0.00210, 0.0105, 0.00501), 3)),
       slope = as.numeric(round(c(0.483, 0.659, 0.646, 0.650), 3))
     )

     gimap_dataset$overall_results$
     ...
     testthat::expect_identical(round(gimap_dataset$gi_scores$mean_gi_score[1], 3), round(-3.841947, 3))
     testthat::expect_identical(round(gimap_dataset$gi_scores$p_val[1], 3), round(0.001203125, 3))
})
