test_that("Test Genetic Interaction score calculations", {
     ...
     testthat::expect_identical(round(gimap_dataset$overall_results$intercept[1], 3),
                                round(results$intercept[1], 3))
     testthat::expect_identical(gimap_dataset$overall_results$rep[1], results$rep[1])

     testthat::expect_identical(round(gimap_dataset$gi_scores$mean_expected_cs[1], 3),
                                round( 0.360, 3))
     testthat::expect_identical(round(gimap_dataset$gi_scores$mean_observed_cs[1], 3),
                                round(-4.411, 3))
     testthat::expect_identical(round(gimap_dataset$gi_scores$mean_gi_score[1], 3),
                                round(-4.0740, 3))
     testthat::expect_identical(round(gimap_dataset$gi_scores$p_val[1], 3),
                                round(0.001203125, 3))
})
